...
kafka_source = (KafkaSource.builder().set_bootstrap_servers('localhost:9094')
    .set_group_id('wfc_flink_ui')
    .set_starting_offsets(KafkaOffsetsInitializer.latest())
    .set_value_only_deserializer(SimpleStringSchema()).set_topics('visits').build())

watermark_strategy = (WatermarkStrategy.for_bounded_out_of_orderness(Duration.of_minutes(3))
                      .with_timestamp_assigner(VisitTimestampAssigner()).with_idleness(Duration.of_minutes(1)))
...
def map_json_to_visit(json_payload: str) -> Visit:
    ...
    return Visit(visit_id=event['visit_id'], browser=event['context']['technical']['browser'], event_time=event_time)
...
records_per_visit_counts: WindowedStream = (data_source.map(map_json_to_visit)
    ...
    .allowed_lateness(0).trigger(EventTimeTrigger()))
